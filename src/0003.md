...
- interface 的使用场景（正是因为有了同名接口会自动合并的特性，在开发第三方库的时候就能够为使用者提供更好的安全保障）
  - 需要利用接口自动合并特性
  - 定义对象类型且无需使用type时
  

[【前端必备】type 和 interface 傻傻分不清楚？](https://www.bilibili.com/read/cv16600117?from=note)
### 工具类型
...
// 通过继承复用并修改 completed 的类型
interface TodoChangeCompleted extends Omit<Todo, "description" | "title"> {
  completed: string
}

const todo1: TodoChangeCompleted = {
  completed: 'false',
};
```
...
```
- keyof any 返回的是联合类型 string | number | symbol 
